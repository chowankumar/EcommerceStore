  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col  items-center w-[90%] sm:max-w-96 mx-auto mt-14 gap-4 text-gray-700'>
      ...
      <div className='w-full flex justify-between text-sm mt-[-8px]'>
        ...
        {
          currentState === 'Login'
            ? <p onClick={() => setCurrentState('Sign')} className='cursor-pointer'>Create account</p>
            : <p onClick={() => setCurrentState('Login')} className='cursor-pointer'>Login Here</p>
        }

      </div>

      <button className='bg-black text-white font-light px-8 py-2 mt-4'>{currentState === 'Login' ? 'Sign In' : 'Sign'}</button>
